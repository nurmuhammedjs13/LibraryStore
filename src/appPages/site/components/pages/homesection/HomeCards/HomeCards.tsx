...
// import { useRouter } from "next/navigation";
// import { useGetBooksQuery } from "@/redux/api/books";
// import { useGetFavoriteQuery, useAddFavoriteItemMutation, useRemoveFavoriteItemMutation } from "@/redux/api/favorite"; // Импортируем мутации
// import scss from "./HomeCards.module.scss";
// import defaultBook from "@/assets/Icons/defaultBook.webp";
// import star0 from "@/assets/Icons/star0.png";
// import star1 from "@/assets/Icons/star1.png";
...
//     const router = useRouter();
//     const { data = [], isLoading, isError } = useGetBooksQuery();
//     const { data: favoriteData = [], isLoading: isLoadingFav, isError: isErrorFav } = useGetFavoriteQuery();
//     const [addFavorite, { isLoading: isAddingFavorite }] = useAddFavoriteItemMutation();
//     const [removeFavorite, { isLoading: isRemovingFavorite }] = useRemoveFavoriteItemMutation();

//     const [likedItems, setLikedItems] = useState<number[]>([]);

//     if (isLoading || isLoadingFav) return <LoadingState />;
//     if (isError || isErrorFav) return <ErrorState />;

//     // Функция для добавления/удаления книги из избранного
//     const toggleLike = async (bookId: number) => {
//         const userId = 1; // Предположим, что мы получаем ID пользователя после авторизации

//         // Проверка существования книги
//         const bookExists = data.some((book: Book) => book.id === bookId);
//         if (!bookExists) {
//             console.error("Книга с таким ID не существует:", bookId);
...
//             (item: { katalog_books_like: number }) =>
//                 item.katalog_books_like === bookId
//         );

//         try {
//             if (isLiked) {
//                 // Если книга уже в избранном, удалим её
...
//             } else {
//                 // Если книга не в избранном, добавим её
//                 await addFavorite({
//                     bookId,
//                     userId,
//                 }).unwrap();
...
//             // Обработка ошибок с дублированием и некорректным ID
//             if (error?.data?.user_favorite?.[0]?.includes("already exists")) {
//                 alert("Эта книга уже добавлена в избранное.");
//             } else if (error?.data?.katalog_books_like?.[0]?.includes("Invalid pk")) {
//                 alert("Ошибка: некорректный ID книги.");
//             } else {
...
//                                 key={book.id}
//                                 book={book}
//                                 isLiked={favoriteData.some(
//                                     (item: { katalog_books_like: number }) =>
//                                         item.katalog_books_like === book.id
//                                 )}
//                                 onLikeToggle={toggleLike}
//                                 onNavigate={navigateToBook}
//                             />
