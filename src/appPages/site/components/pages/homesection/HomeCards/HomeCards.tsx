...
const HomeCards: React.FC = () => {
  ...
  const toggleCart = async (book: Book) => {
    ...
    try {
      // Проверяем, есть ли книга уже в корзине пользователя
      const isInCart = cartData?.some(
        (item) => item.books.id === book.id && item.user === userId
      );
                ...
                <div className={scss.cardInfo}>
                  <div className={scss.rating}>
                    <Image
                      ...
                      src={
                        STAR_RATINGS[Math.floor(book.average_rating)] || star0
                      }
                  ...
                  <div className={scss.confirm}>
                    ...
                    <div className={scss.actions}>
                      ...
                      >
                        {cartData?.some(
                          (item) =>
                            item?.books.id === book.id && item.user === userId
                        )
                          ? "В корзине"
                          : "В корзину"}
                      </button>
