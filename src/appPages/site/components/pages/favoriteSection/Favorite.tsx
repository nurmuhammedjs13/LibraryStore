          ...
          <div className={scss.cards}>
            {favoriteData.map((favorite) => (
              <div key={favorite.id} className={scss.card}>
                <Image
                  onClick={() => navigateToBook(favorite.books_like.id)}
                  width={150}
                  height={200}
                  quality={80}
                  className={scss.bookImage}
                  src={favorite.books_like.book_images[0]?.book_images || defaultBook}
                ...
                <div className={scss.cardInfo}>
                  <div className={scss.rating}>
                    <Image
                      ...
                      src={
                        STAR_RATINGS[favorite.books_like.average_rating] || star0
                      }
                      alt={`Рейтинг: ${favorite.books_like.average_rating || 0} звёзд`}
                    />
                  </div>
                  <h2 className={scss.name}>{favorite.books_like.book_name}</h2>
                  <h3 className={scss.author}>{favorite.books_like.author}</h3>
                  <p className={scss.genre}>
                    Жанр:{" "}
                    {favorite.books_like.janre.map((genre, i) => (
                      <span key={genre.janre_name} className={scss.janre}>
                        {genre.janre_name}
                        {i < favorite.books_like.janre.length - 1 && ", "}
                      </span>
                    ))}
                  </p>
                  <div className={scss.confirm}>
                    ...
                    <div className={scss.actions}>
                      <button
                        ...
                        onClick={() => addToCart(favorite.books_like.id)}
                      >
                        {cartItems.includes(favorite.books_like.id)
                          ? "В корзине"
                          : "В корзину"}
                      </button>
                      <button
