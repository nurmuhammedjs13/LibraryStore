  ...
  return (
    <div className={scss.card}>
      <Image
        ...
        src={
          favorite.katalog_books_like.books.book_images[0]?.book_images ||
          defaultBook
        }
        ...
        <p className={scss.genre}>
          ...
          {favorite.katalog_books_like.books.janre.map((genre, i) => (
            <span key={genre.janre_name} className={scss.janre}>
              {genre.janre_name}
              {i < favorite.katalog_books_like.books.janre.length - 1 && ", "}
            </span>
          ))}
        </p>
...
const Favorite: React.FC = () => {
  ...
  const { data: favoriteData = [], isLoading, isError } = useGetFavoriteQuery();
