...
const PlacinganOrder = () => {
  ...
  const handleSubmitOrder = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!formData.lastName) validationErrors.push("Фамилия обязательна");
      if (!formData.email) validationErrors.push("Email обязателен");
      if (!formData.phone) validationErrors.push("Номер телефона обязателен");
      ...
      if (!formData.receipt) {
        ...
        if (formData.firstName.trim().length > 32)
            validationErrors.push("Имя должно содержать не более 32 символов");
        if (formData.lastName.trim().length > 32)
            validationErrors.push("Фамилия должна содержать не более 32 символов");
        if (formData.address && formData.address.trim().length > 255)
            validationErrors.push("Адрес должен содержать не более 255 символов");
