...
const MainCatalog: React.FC = () => {
  ...
  const filteredBooks = (
    ...
    return matchesGenre && matchesMinPrice && matchesMaxPrice && matchesRating;
  });
      ...
      <div className="container">
        <div className={scss.content}>
          ...
          >
              ...
              <div className={scss.genres}>
                {genres?.map((genre) => (
                  <h1
                    ...
                    onClick={() => handleGenreClick(genre.janre_name)}
                  >
                    {genre.janre_name}
                  </h1>
                ))}
            ...
            <button onClick={handleFilterClick} className={scss.searchButton}>
              Найти
            </button>
          </div>
          <div className={scss.CardBlock}>
            <div className={scss.cardsNav}>
              ...
              <button onClick={resetAllFilters} className={scss.filtButton}>
                ✕
              </button>
            </div>
                            ...
                            {item.isDiscountBook ? (
                              <>
                                {calculateDiscountedPrice(
                                  Math.ceil(item.price / 10) * 10,
                                  item.discount
                                )}
                                c<span className={scss.originalPrice}></span>
                              </>
                            ) : (
                              `${Math.ceil(item.price / 10) * 10}c`
                            )}
                            {item.discount > 0 && (
